...
class ExportPlotForStationsMixin:
    ...
    def export_plots(self):
        ...
        for i, row in self.df_stations.iterrows():
            ...
            # Use a copy so pio.write_image (kaleido) can't mutate self.fig.layout
            # with injected pixel dimensions that would break the live UI.
            fig_copy = go.Figure(self.fig)
            fig_copy.layout = self.layout
            fig_copy.layout.title = station_name
            pio.write_image(fig_copy, fpath_plot, scale=3)
