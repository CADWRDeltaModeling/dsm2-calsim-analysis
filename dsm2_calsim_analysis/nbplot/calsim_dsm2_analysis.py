import pandas as pd
import numpy as np
import dsm2_calsim_analysis.utilities as pdmu
import dsm2_calsim_analysis.nbplot as pdmn
import yaml
import pathlib
import logging
...
def main(config_file):
    ...
    Returns:
        A dashboard with a dropdown to select different views
    """
    # Placeholder panel shown while plots are building
    view_area = pn.Column()
    dd_view = pn.widgets.Select(name="Select Analysis Panel", options=["Loading..."])
    mainPanel = pn.Column(dd_view, view_area)
    ...
    for plot_config in config["plots"]:
        ...
        try:
            ...
            plot = plot_func(*args)
            views[options["title"]] = pn.Column(
                pn.pane.Markdown(f"## {options['title']}"), plot
            )
            if not view_area.objects:
                view_area.objects = [views[options["title"]]]
        except Exception as e:
            import traceback

            message = f"{e}\n{traceback.format_exc()}"
            views[options["title"]] = pn.pane.Str(message)
            continue

    # Populate dropdown and wire up the view switcher
    dd_view.options = list(views.keys())
    if dd_view.options:
        dd_view.value = dd_view.options[0]
        view_area.objects = [views[dd_view.value]]

    def on_view_change(event):
        view_area.objects = [views[event.new]]

    dd_view.param.watch(on_view_change, "value")
    
    dd_view.value = dd_view.options[0]  # trigger initial view
