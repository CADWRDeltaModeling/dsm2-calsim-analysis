...
__all__ = [
    ...
    "read_calsim3_wateryear_types",
    "read_calsim_sacvalley_table",
    "read_regulations",
    "read_D1641FWS_conditional",
    "read_dss_to_df",
    "generate_regulation_timeseries",
    "generate_regulation_timeseries_calsim",
    "prep_df",
    "calculate_days_standard_met",
]
...
def read_regulations(fpath, df_wyt):
    ...
    df[["month", "date"]] = df[["month", "date"]].map(np.int64)
    ...
    for loc in locs:
        ...
        for wy in range(yr_start, yr_end + 1):
            ...
            for i, row in df[mask].iterrows():
                ...
                regs.append(val)
        dfs.append(
            pd.DataFrame(data={"value": regs, "location": loc}, index=timestamps)
        )
...
def generate_regulation_timeseries(df_reg, df, freq=None):
    ...
    for station in df_reg["location"].unique():
        ...
        mask = (df_ec_reg_ts["time"] >= t_begin) & (df_ec_reg_ts["time"] <= t_end)
...
def read_dss_to_df(
    ...
    for path in paths:
        ...
        if fparts_to_read is not None and parts[6] not in fparts_to_read:
            continue
        data, cunits, ctype = dssfile.read_rts(path, start_date_str, end_date_str)
        try:
...
def generate_regulation_timeseries_calsim(fpath, freq="1MON"):
    ...
    df_stds = read_dss_to_df(
        fpath,
        bparts_to_read=stations,
        cparts_to_read=["SALINITY"],
        eparts_to_read=[freq],
    )
    ...
    df_stds["value"] = np.where(
        df_stds["value"] > 99000 - 1, 0, df_stds["value"]
    )  # Calsim 3 uses 99000 for non-regulation period
    df_stds["value"] = np.where(
        (df_stds["station"] == "RSAN018")
        & (df_stds["time"].dt.month.isin([9, 10, 11, 12, 1, 2, 3])),
        0,
        df_stds["value"],
    )  # Calsim 3 has guidance standard for JER out of Apr-Aug

    ec_250chlr = (
        df_stds[df_stds["station"] == "ROLD024"]["value"].unique().max()
    )  # convert Chloride to EC
    df_stds["value"] = np.where(
        (df_stds["station"] == "ROLD024"), ec_250chlr, df_stds["value"]
    )  # only check standard of Chloride 250
...
def prep_df(scenarios, sta, var, intvl, df_wyt, period, src="ALL"):
    ...
    df = df.assign(
        station=ds_parts.map(lambda x: x[2]),
